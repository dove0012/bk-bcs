...
// GetCloudPublicIPs implements interface cmproto.ClusterManagerServer
func (cm *ClusterManager) GetCloudPublicIPs(ctx context.Context,
	req *cmproto.GetCloudPublicIPsRequest, resp *cmproto.GetCloudPublicIPsResponse) error {
	reqID, err := requestIDFromContext(ctx)
	if err != nil {
		return err
	}
	...
	start := time.Now()
	ca := cloudvpc.NewGGetCloudPublicIPsAction(cm.model)
	ca.Handle(ctx, req, resp)

	metrics.ReportAPIRequestMetric("GetCloudPublicIPs", "grpc", strconv.Itoa(int(resp.Code)), start)
	blog.Infof("reqID: %s, action: GetCloudPublicIPs, req %v, resp.Code %d, "+
		"resp.Message %s, resp.Data %v", reqID, req, resp.Code, resp.Message, resp.Data) // nolint
	blog.V(5).Infof("reqID: %s, action: GetCloudPublicIPs, req %v, resp %v", reqID, req, resp)
	return nil
}
